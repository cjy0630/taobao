    ...
    def parse(self, response):
        ...
        try:
            ...
            for data in datas:
                ...
                # print(item_id, user_id, title, url)
                ...
                yield scrapy.Request(comment_url, headers=headers, meta={'referer': url, 'item_id': item_id, 'user_id': user_id}, callback=self.parse_comment, dont_filter=True)
            self.offset += 44
            cookie = get_cookie()
            headers = {
                'cookie': cookie,
    ...
    def parse_comment(self, response):
        ...
        try:
            ...
            for data in datas:
                ...
                print('商品ID：%s  商家ID：%s  评价内容：%s  评价时间：%s' % (response.meta['item_id'], response.meta['user_id'], rate_content, rate_date))
